...
class Query(graphene.ObjectType):
    ...
    def resolve_rooms(cls, info, **kwargs):
        ...
        if fake_users.exists() and worker_room_id:
            ...
            exclude_filters = exclude_filters | Q(user_id__in=fake_users, deleted=1) | Q(target__in=fake_users, deleted=2)
            sub_exclude_filters = {'user_id__in': fake_users}
        else:
            filter_query = filter_query | Q(user_id=user) | Q(target=user)
            exclude_filters = exclude_filters | Q(user_id=user, deleted=1) | Q(target=user, deleted=2)
